...
#define PATH "/mnt/idata03/data/run-%ddata-%d"
...
int main(int argc, const char* argv[])
{
    ...
    cin>>start_run;
    cin>>stop_run;
    cout<<"Requested run range: "<<start_run<<" to "<<stop_run<<endl;

    if(start_run> stop_run || start_run<0 || stop_run<0)
    {
    ...
    {
	cout << "Analyzing run number: "<<run<<flush;
	...
	for( daq=21; daq<25; daq++)
	{
	    ...
	    sprintf(data_file,PATH,run, daq);
	    ifstream runData(data_file);

	    if (!runData) 
	    {
		cout << "File does not exist for run: "<<run<<" Module: "<< daq<<endl;
		continue;
	    }
	    else
	    {
		daq_status=process(data_file,run, daq);
